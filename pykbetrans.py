...
root = 'D:\PYSCT\ToTranslate'
newRoot = 'D:\PYSCT\Translated'
...
def replaceFirstWordPy(strToCheck):
	strToCheck = strToCheck.replace("pas", "pass")
	strToCheck = strToCheck.replace("</root", "</root>")
	return strToCheck
	...
	for name in files:
		...
		if name.lower().endswith(patterns):
			...
			if skipSpecialEncodedFiles:
				if newEncoding != chineseEncoding or newEncoding != utfEncoding or newEncoding != chineseSuperSetEncoding:
					continue
			...
			for line in file:
				...
				if containsChineseCharacter(line):
					...
					#MOST OF THIS IS UNUSED FOR NOW
